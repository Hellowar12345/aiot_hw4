...
train_mobilenet.py
==================
雿輻 PyTorch + MobileNetV2 (Transfer Learning) 閮毀?喲?芸?撣?+ Error ??憿芋??- 閮毀鞈?嚗ataset/train/ (rock, paper, scissors)
- 皜祈岫鞈?嚗ataset/test/  (rock, paper, scissors)
- Error 憿嚗????閮蔣??朣洵??
- 頛詨??嚗ccuracy?recision?ecall?1-score (macro)
- 閮毀摰敺?璅∪?摮 demo/rps_mobilenet.pth
"""

import sys
import io
# 撘瑕 UTF-8 頛詨嚗圾瘙?Windows CP950 蝺函Ⅳ??嚗?sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace', line_buffering=True)
...
# ?踵? Classifier -> 4 憿撓??in_features = model.classifier[1].in_features
