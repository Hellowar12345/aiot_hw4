...
train_resnet.py
======================
使用 PyTorch + ResNet-18 (Transfer Learning) 訓練石頭剪刀布 + Error 四分類模型
...
- 訓練完畢後將模型存至 demo/rps_resnet.pth
"""
...
IMG_SIZE     = 224
BATCH_SIZE   = 32
NUM_EPOCHS   = 3  # 保持與之前相同的 Epoch 數方便比較
LR           = 1e-4
DEVICE       = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

BASE_DIR  = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
# 如果你想使用剛剛用 MediaPipe 裁切好加黑邊的完美資料集，可以把這裡改成 'dataset_mediapipe'
TRAIN_DIR = os.path.join(BASE_DIR, 'dataset', 'train')
TEST_DIR  = os.path.join(BASE_DIR, 'dataset', 'test')
DEMO_DIR  = os.path.join(BASE_DIR, 'demo')

print("=" * 55)
print("  ResNet-18 Rock-Paper-Scissors Training")
print("=" * 55)
...
# 4. 建立 ResNet-18 模型
# ─────────────────────────────────────────────
print("[Step 2] Building ResNet-18 (ImageNet pretrained)...")
...
# 替換 Classifier (ResNet 最後一層是 fc) -> 4 類輸出
...
# 確保新的分類層可以進行梯度更新 (雖然預設就是 True，但保險起見再設定一次)
...
save_path = os.path.join(DEMO_DIR, 'rps_resnet.pth')
