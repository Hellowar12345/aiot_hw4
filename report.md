...
*(註：MLP 因為直接使用骨架座標特徵，排除了背景與光線干擾，因此在測試集上達到近乎完美的準確率。)*

### 4.2 推論效能比較（Raspberry Pi）
...
2. **Demo 程式撰寫與優化**：AI 協助撰寫了 `camera_landmark_mlp.py`，完整實作了「特徵正規化 → MLP 推論 → UI 渲染」的 pipeline。
